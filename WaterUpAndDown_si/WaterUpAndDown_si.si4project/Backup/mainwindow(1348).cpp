...
Mat DownAndWaterDownFrame(IMAGEHEIGHT, IMAGEWIDTH, CV_8UC3);
Mat MergeFramet;
...
ChangeMode_Type ChangeMode = Mode_Splice_Play/*Mode_3Video_Play*/;//切换模式
...
//开始图片拼接*********************************************************************************                    
            ...
            //创建拼接后的图,需提前计算图的大小
            int UpAndWaterUpFrame_width; //取最右点的长度为拼接图的长度
...
//            OptimizeSeam(MidFrameUp, imageTransform1, UpAndWaterUpFrame);
            ...
            imwrite("UpAndWaterUpFrame.jpg", UpAndWaterUpFrame);
            imwrite("DownAndWaterDownFinish.jpg", DownAndWaterDownFrame);
            
#endif
            Mat MergeFrame(DownAndWaterDownFrame.rows + UpAndWaterUpFrame.rows- IMAGEHEIGHT/2, DownAndWaterDownFrame.cols, CV_8UC3);   
            UpAndWaterUpFrame.copyTo( MergeFrame( Rect( 0, 0,UpAndWaterUpFrame.cols, UpAndWaterUpFrame.rows) ) );
            DownAndWaterDownFrame.copyTo( MergeFrame( Rect( 0, UpAndWaterUpFrame.rows - IMAGEHEIGHT/2,DownAndWaterDownFrame.cols, DownAndWaterDownFrame.rows) ) );
#ifndef     Release
            imwrite("MergeFrame.jpg", MergeFrame);
#endif
            MergeFramet = MergeFrame;
//*********************************************************************************
            emit MergeFinish();
//            cvtColor(MergeFrame, temp, CV_BGR2RGB);
...
void MainWindow::Pt_MidDown_ThreadCallBack(void)
{
    while(1)
    {
        cout << "MidFrameDivideFinish" << endl;
...
//            ui->CameraDown_label->show();
            ...
            Mat WaterFrame_C, temp, Pic_CutWater;
            cout << "WaterFrameStart" << endl;
...
#ifndef     Release
            imwrite("WaterFrame.jpg", WaterFrame);
            imwrite("WaterFrame_C.jpg", WaterFrame_C);
            imwrite("WaterFrame_C1.jpg", WaterFrame_C(ROIRect));
#endif         
            transpose(WaterFrame_C, temp);
        	flip(temp, WaterFrame_C, 0);//逆时针旋转

            int num = ( ui->lineEdit_SurfNum_2->text() ).toInt();
//开始图片拼接*********************************************************************************                    
...
#ifndef     Release
            Mat outimg;                                //drawMatches这个函数直接画出摆在一起的图
            drawMatches(WaterFrame_C, MidFrameDown_key, MidFrameWater, key1, good_matches, outimg, Scalar::all(-1), Scalar::all(-1), vector<char>(), DrawMatchesFlags::NOT_DRAW_SINGLE_POINTS); //绘制匹配点
            ...
            Mat homo = findHomography(imagePoints1, imagePoints2, CV_RANSAC);
            //计算配准图的四个顶点坐标
            CalcCorners(homo, MidFrameWater);
            //图像配准
            Mat imageTransform1, imageTransform2;
...
//            Mat DownAndWaterDownFrame(DownAndWaterDownFrame_height, DownAndWaterDownFrame_width, CV_8UC3);
//            DownAndWaterDownFrame.rows = DownAndWaterDownFrame_height;
//            DownAndWaterDownFrame.cols = DownAndWaterDownFrame_width;
//                
//            DownAndWaterDownFrame.setTo(0);
//
//            imageTransform1.copyTo(DownAndWaterDownFrame(Rect(0, 0, imageTransform1.cols, imageTransform1.rows)));
//            // Pic_CutWater.copyTo(MidFrameUp(WaterRect));
//            MidFrameWater.copyTo(DownAndWaterDownFrame(Rect(0, 0, MidFrameWater.cols, MidFrameWater.rows)));
...
//*********************************************************************************
            //完成一次图片拼接
            cout << "DownAndWaterDownFinish" << endl;
